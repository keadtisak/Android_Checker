...
/**
 * Created by CaptainCom on 5/25/2015.
 */
public class ProcessPin{

    public boolean processPin(Mat image, int col1, int row2, int col2){

        Rect roi = new Rect(0,col1,row2,col2);
        Mat croppin=new Mat(image,roi);
        Highgui.imwrite(Environment.getExternalStorageDirectory() + "/DCIM/CameraSnap/CropPin" + Tutorial3View.sum + ".jpg", croppin);
       ...
       Log.i("Pin Cols  ", Arrays.toString(sumCols));
       Log.i("Pin rows ", Arrays.toString(sumRows));
       arrrow =setRow(sumCols);
        //lisr =setRow(sumCols);
        arrcol = setCol(sumRows);
        Log.i("Pin Row  ", Arrays.toString(arrrow));
        Log.i("Pin Cols  ", Arrays.toString(arrcol));
    ...
    public boolean  Croppin(Mat img , int startX,int startY,int endX,int endY,int i){
        ...
        Highgui.imwrite(Environment.getExternalStorageDirectory() + "/DCIM/CameraSnap/CropsubRowPin" + Tutorial3View.sum +""+i+ ".png", cropch);
        CheckPin ch = new CheckPin();
        boolean con = ch.checkpin(cropch,i);
